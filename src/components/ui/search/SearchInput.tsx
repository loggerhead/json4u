...
export default function SearchInput<T extends { id: string }>({
  ...
  const height =
    items.length > 0 && itemHeightRef.current > 0
      ? Math.min(items.length * itemHeightRef.current + 2 * commandGroupPaddingY, searchMenuHeight)
                  ...
                  <div
                    ...
                    ref={(el) => {
                      ...
                      if (itemHeightRef.current === 0) {
                        itemHeightRef.current = el?.getBoundingClientRect().height ?? 0;
                      }
                    }}
                  >
                    <CommandItem
