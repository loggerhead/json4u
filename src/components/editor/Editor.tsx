...
import { EditorWrapper, type Kind } from "@/lib/editor";
...
function MyEditor({ kind, ...props }: EditorProps) {
  ...
  return (
    <MonacoEditor
      ...
      onMount={(editor) => {
        new EditorWrapper(editor, kind, stores).init();
        stores.editorStore.getState().setTranslations(translations);
      }}
      onChange={(value, ev) => {
        const editor = stores.editorStore.getState()[kind];
