...
import {setFocusLeft, setFocusRight, setStatusBar, showRightEditor} from "@/features/ctxSlice";
...
export class EditorRef {
  constructor(store, dispatch, editor, monaco) {
    // redux
    this.store = store;
  ...
  minify() {
    ...
    if (node?.length === text.length || !node.hasError()) {
      text = node.stringify();
    } else {
  ...
  // 解析 JSON 并将解析结果设置到 editor
  parseAndSetText(options = {}) {
    let text = this.text();
   ...
   * @returns {boolean} 解析成功了吗？
   */
  sort() {
    /** @type {ParseOptions} */
    const options = {
      nest: ctx(this.store).enableNestParse,
      format: ctx(this.store).enableAutoFormat,
      order: "asc",
    };
    return this.parseAndSetText(options);
  }
  ...
  onPaste(e = null) {
    ...
    const needNestParse = ctx(this.store).enableNestParse;
    const needAutoSort = ctx(this.store).enableAutoSort;
    const needAutoFormat = ctx(this.store).enableAutoFormat;
    /** @type {ParseOptions} */
    const options = {
      nest: needNestParse,
      format: needAutoFormat,
    };
    let formatted = false;
    ...
    } else if (needNestParse) {
      formatted = this.parseAndSetText(options);
    }

    // sort 已经进行过一次 format，所以如果 sort 成功，则不需要再 format
    if (needAutoFormat && !formatted) {
      const text = format.format(this.text());
      this.setText(text);
    }
    ...
    if (this === this.rightEditor) {
      ...
      if (this.leftEditor?.text().length && this.rightEditor?.text().length) {
        this.compare(needAutoSort);
      }

      // 展开右侧编辑器
      this.dispatch(showRightEditor());
    }

    // 测量时间
    const cost = performance.now() - now;
    if (cost >= 100) {
      console.log(`paste ${textLength} chars cost: ${Math.round(cost)}ms`);
    }
  }
  ...
  registerDidFocus() {
    this.editor.onDidFocusEditorText(() => {
      this.dispatch(this === this.leftEditor ? setFocusLeft() : setFocusRight());
    });
  }
