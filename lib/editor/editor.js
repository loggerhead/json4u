...
export class EditorRef {
  ...
  setStatusBarText(text, ...ids) {
    ...
    if (text) {
      // 计算需要展示的背景色
      [pureText, colors] = color.matchColorTags(text);
      tagColor = color.max(colors);
    }
    ...
    for (let id of ids) {
      ...
      text = text ? `<${tagColor}>${prefix}${pureText}</${tagColor}>` : "";
      o[id] = text;
    }
  ...
  resetDiffDecorations() {
    ...
    this.diffs = [];
    this.diffIndex = 0;
    this.setStatusBarText("", EditorRef.StatusBarDiff, EditorRef.StatusBarCompare);
  }
