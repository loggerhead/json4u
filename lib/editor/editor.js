...
export class EditorRef {
  ...
  compare() {
    ...
    // 进行比较
    const dr = compare.smartCompare(ltext, rtext);
  ...
  highlight() {
    ...
    // 如果是文本比较，生成 diff 填充块
    if (this.diffResult.isTextCompare) {
      //   const leftRanges = removeInlineRanges(this.leftEditor.model(), this.diffResult.leftBlockRanges);
      //   const rightRanges = removeInlineRanges(this.rightEditor.model(), this.diffResult.rightBlockRanges);
      //   const [leftFills, rightFills] = generateFillRanges(leftRanges, rightRanges);
