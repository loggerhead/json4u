...
import {setFocusLeft, setFocusRight, setLastAction, setStatusBar} from "@/reducers";
import {
  StatusBarCompare,
  StatusBarLeftValidate,
  StatusBarPath,
  StatusBarPos,
  StatusBarRightValidate,
  StatusBarSetting,
  StatusBarValidate,
} from "@/components/statusBar";

export class EditorRef {
  constructor(side, store, dispatch, editor) {
    this.side = side;
    // redux
    this.store = store;
  ...
  setStatusBarText(text, id, blink = false) {
    // 计算需要展示的背景色
    const [_, colors] = color.matchTags(text);
  ...
  resetDiffDecorations() {
    ...
    this.dispatch(setLastAction(null));
  }
  ...
  csv2json(header = false) {
    const data = {
      text: this.text(),
      options: {
        ...this.genOptions(),
        withCsvHeader: header,
      },
    };

    this.call("csv2json", data, (self, data) => {
      const {text, error} = data;

      if (error) {
        Message.error(`CSV 解析出错: ${error}`);
      } else {
        self.setText(text);
      }
    });
  ...
  json2csv() {
    ...
    this.call("json2csv", data, (self, data) => {
      const {text, errors} = data;
      const errmsg = self.genErrorAlert(errors, false);

      if (errmsg) {
        Message.error(`JSON 转 CSV 失败: ${errmsg}`);
      } else {
        self.setText(text);
      }
    });
  ...
  compare(options = {}, callback = null) {
    ...
    this.call("compare", data, (self, data) => {
      EditorRef.handleCompareResult(self, data);
  ...
  onPaste(text = undefined) {
    ...
    this.call("handlePaste", data, (self, data) => {
      const {text, errors, diffResult} = data;
      self.setText(text);
  ...
  showDiffMsg() {
    ...
    this.dispatch(setLastAction({
      action: 'Compare',
      options: {
        ...this.genOptions(),
        needTextCompare: this.diffResult.isTextCompare,
      },
    }));
  }
  ...
  registerMenuItem(name, fnName, groupName, order, keybindings = []) {
    const item = {
      id: fnName,
      label: name,
      // 决定菜单项展示的顺序。除了 navigation 是固定第一个展示以外，其他都按字母序排列
  ...
  registerMenuItems() {
    ...
    register("上传文件", "uploadFile", "1");
    register("下载文件", "downloadFile", "1");
    register("上一个差异", "jumpToPrevDiff", "2", [editorApi.KeyMod.Alt | editorApi.KeyCode.KeyP]);
    register("下一个差异", "jumpToNextDiff", "2", [editorApi.KeyMod.Alt | editorApi.KeyCode.KeyN]);

    // 注册折叠菜单项
    for (let level = 2; level <= 7; level++) {
      this[`foldLevel${level}`] = () => this.editor.trigger("regionFold", `editor.foldLevel${level}`);
      register(`折叠层级${level}`, `foldLevel${level}`, "3", [
    ...
    register("展开全部折叠区域", "unfoldAll", "3", [editorApi.KeyMod.Alt | editorApi.KeyCode.Digit0]);
  }
  ...
  // 点击右键菜单项后下载文件
  downloadFile() {
    const fileName = `${this.side}.json`;
    const fileContent = this.text();
  ...
  registerPositionChange() {
    this.editor.onDidChangeCursorPosition((e) => {
      ...
      const loc = jsonc.getLocation(text, offset);
      const {lineNumber, column} = e.position;
      const selectionLength = model.getValueInRange(this.editor.getSelection()).length;

      const msg = `${lineNumber}:${column}` + (selectionLength > 0 ? ` (${selectionLength} 个字符)` : "");
      this.setStatusBarText(msg, StatusBarPos);

      if (loc.path) {
        this.setStatusBarText(pointer.toArray(loc.path), StatusBarPath);
      }
    });
  }
   ...
   * @param {boolean} withTag 需要生成 tag 吗？
   * @returns {string}
   */
  genErrorAlert(errors, withTag = true) {
    if (!errors?.length) {
      return "";
    }
    ...
    if (typeof error === "string") {
      ...
      const {offset, length, contextTexts} = error;
      ...
      if (withTag) {
        ...
        return `${startLineNumber} 行 ${startColumn} 列解析错误: ${left}${middle}${right}`;
      }
    }
  }
      ...
      return {
        ...
        options: {
          ...
          minimap: {
            ...
            position: editorApi.editor.MinimapPosition.Inline,
          },
          // 高亮 minimap 右侧的 overview ruler
          overviewRuler: {
            color: color.getOverviewRulerColor(diffType),
            position: editorApi.editor.OverviewRulerLane.Full,
          },
        },
      };
    }
