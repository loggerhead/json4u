...
export class EditorRef {
  ...
  sort(options = {}) {
    let text = this.text();
    const [tree, errors] = parser.parseJSON(text, options?.nest);
    const errmsg = this.genErrorAlert(errors);

    if (errmsg) {
      this.setStatusBarText(errmsg, StatusBarValidate);
    } else {
      text = tree.stringify(options?.reverse ? "desc" : "asc");
  ...
  onPaste(e = null) {
    ...
    if (e === null || e.range.equalsRange(this.model().getFullModelRange())) {
      ...
      if (needAutoSort) {
        formatted = this.sort({nest: needNestParse});
        // 自动排序时，已经进行过嵌套解析，因此不需要再解析
      } else if (needNestParse) {
        const [node, errors] = parser.parseJSON(this.text(), needNestParse);
        // 解析成功时，进行格式化
        if (errors.length === 0) {
          const text = format.format(node.stringify());
          this.setText(text);
          formatted = true;
        }
      }
