...
// 当 (行内差异 size / 块差异 size) 超过指定比率时，不高亮对应块差异的行内差异
const hideHighlightWhenRateExceeds = 0.8;
// 当块差异 size 小于指定长度时，高亮对应块差异的行内差异
const showHighlightWhenSizeLess = 20;
...
export class EditorRef {
  ...
  setStatusBarText(text, ...ids) {
    ...
    for (let id of ids) {
      ...
      if (id === StatusBarValidate) {
        id = this === this.leftEditor ? StatusBarLeftValidate : StatusBarRightValidate;
  ...
  resetDiffDecorations() {
    const clear = (editor) => {
      ...
      editor.setStatusBarText("", StatusBarDiff, StatusBarCompare);
  ...
  jumpToDiff(direction = 0) {
    ...
    this.jumpToDiffID = setTimeout(() => {
      ...
      if (range) {
        ...
        this.setStatusBarText(`第 ${index + 1} 个差异`, StatusBarDiff);
      } else {
        this.setStatusBarText("", StatusBarDiff);
      }
    }, 100);
  }
  ...
  showResultMsg() {
    ...
    const msg = msgs.join("。");
    this.setStatusBarText(msg, StatusBarCompare);
  }
  ...
  highlight() {
    const genDecoration = (hunkDiff) => {
      ...
      // 计算行内差异 size 占比
      const inlineRate = hunkDiff.inlineDiffs.reduce((acc, d) => {
        return acc + d.length;
      }, 0) / hunkDiff.length;

      const decoration = editor.newHighlightDecoration(hunkDiff.range, false, color.getColorClass(type, false));
      const inlineDecorations = hunkDiff.inlineDiffs.map((d) => {
        return editor.newHighlightDecoration(d.range, true, color.getColorClass(type, true));
      });
      return [decoration, inlineDecorations, inlineRate];
    };
    ...
    const addDecorations = (type, decoration, inlineDecorations = []) => {
      (type === compare.DEL ? lDecorations : rDecorations).push(decoration, ...inlineDecorations);
    };
    ...
    for (let i = 0; i < hunkDiffs.length;) {
      ...
      const [decoration, inlineDecorations, inlineRate] = genDecoration(hunkDiff);

      if (typeNext && type === compare.DEL && typeNext === compare.INS) {
        const [decorationNext, inlineDecorationsNext, inlineRateNext] = genDecoration(hunkDiffNext);

        if ((inlineRate < hideHighlightWhenRateExceeds && inlineRateNext < hideHighlightWhenRateExceeds)
          || (hunkDiff.length + hunkDiffNext.length < showHighlightWhenSizeLess)) {
        ...
        if (inlineRate < hideHighlightWhenRateExceeds) {
          addDecorations(type, decoration, inlineDecorations);
        } else {
          addDecorations(type, decoration);
        }
