...
export class EditorRef {
  static StatusBarValidate = 100; // 虚拟状态栏 ID
  static StatusBarPath = 0;
  static StatusBarLeftValidate = 1;
  static StatusBarRightValidate = 2;
  static StatusBarSetting = 3;
  static StatusBarCompare = 4;
  static StatusBarDiff = 5;

  constructor(editor, monaco, statusBar, setStatusBar, adjustWidth) {
    // monaco editor 实例
    this.editor = editor;
    // monaco 实例
    this.monaco = monaco;
  ...
  setStatusBarText(text, id) {
    if (text) {
      // 计算需要展示的背景色
      const [pureText, colors] = color.matchColorTags(text);
      const tagColor = color.max(colors);
      let prefix = "";

      // 生成前缀
      if (id === EditorRef.StatusBarValidate) {
        prefix = this === this.leftEditor ? "左侧" : "右侧";
        id = this === this.leftEditor ? EditorRef.StatusBarLeftValidate : EditorRef.StatusBarRightValidate;
      }

      text = `<${tagColor}>${prefix}${pureText}</${tagColor}>`;
    }

    const o = { ...this.statusBar };
    o[id] = text;
    this?.setStatusBar(o);
  }
  ...
  resetDiffDecorations() {
    ...
    this.setStatusBarText("", EditorRef.StatusBarCompare);
  }
  ...
  revealLine(lineNumber) {
    ...
    const pos = { lineNumber: lineNumber, column: 1 };
  ...
  showResultMsg(diffs, isTextCompare, errors) {
    ...
    if (diffs.length == 0) {
      ...
      msgs.push(`<blue>${delN} 删除 ${insN} 新增</blue>`);
    }
  ...
  highlight(leftEditor, rightEditor, diffs) {
    const genDecorations = (diffs, wantType) => {
      const decorations = [];
      const wholeLines = new Set();
      const inlineLines = new Set();
      diffs = diffs.filter((d) => d.type === wantType);
      ...
      for (const { type, offset, length, highlightLine } of diffs) {
        ...
        const d = editor.newHighlightDecoration(range, highlightLine, colorClass);

        for (let l = range.startLineNumber; l <= range.endLineNumber; l++) {
          (highlightLine ? wholeLines : inlineLines).add(l);
        }

        decorations.push(d);
      }

      // 行内差异需要高亮对应行，并且不能重复高亮，否则因为存在透明度，颜色叠加后会加深
      const excluded = [...inlineLines].filter((l) => !wholeLines.has(l));

      for (const l of excluded) {
        const colorClass = color.getColorClass(wantType, true);
        const editor = wantType == DEL ? leftEditor : rightEditor;
        const range = new monaco.Range(l, 1, l, 1);
        const d = editor.newHighlightDecoration(range, true, colorClass);
        decorations.push(d);
      }
    ...
    leftEditor.applyDecorations(genDecorations(diffs, DEL));
    rightEditor.applyDecorations(genDecorations(diffs, INS));
  }
  ...
  registerMenuItems() {
    ...
    for (let level = 2; level <= 4; level++) {
      this[`foldLevel${level}`] = () => this.editor.trigger("fold", `editor.foldLevel${level}`);
      register(`折叠层级${level}`, `foldLevel${level}`, "fold", [monaco.KeyMod.Alt | (monaco.KeyCode.Digit0 + level)]);
    }

    // 注册展开折叠菜单项
    this.unfoldAll = () => this.editor.trigger("fold", "editor.unfoldAll");
    register("展开全部折叠区域", "unfoldAll", "fold", [monaco.KeyMod.Alt | monaco.KeyCode.Digit0]);
  }
