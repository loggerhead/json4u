...
import { semanticCompare, SemanticDiffResult, DEL, INS } from "../diff";
...
export class EditorRef {
  ...
  constructor(editor, monaco, setStatusBar, adjustWidth) {
    ...
    // 比较后的差异
    this.diffResult = new SemanticDiffResult();
  ...
  resetDiffDecorations() {
    const clear = (editor) => {
      ...
      editor.diffIndex = 0;
      editor.diffResult = new SemanticDiffResult();
    };
  ...
  compare() {
    ...
    diffResult.genRanges(this.leftEditor, this.rightEditor);
    this.leftEditor.diffResult = diffResult;
    this.rightEditor.diffResult = diffResult;
  ...
  // 跳转到上一个差异
  jumpToPrevDiff() {
    this.diffIndex = this.jumpToDiff(this.diffIndex, -1);
  }
  ...
  // 跳转到指定差异
  jumpToDiff(index, direction = 0) {
    clearTimeout(this.jumpToDiffID);
    const ranges = this === this.leftEditor ? this.diffResult.leftBlockRanges : this.diffResult.rightBlockRanges;
    index += direction;
  ...
  showResultMsg() {
    const msgs = [];
    const delN = this.diffResult.leftBlockRanges.length;
    const insN = this.diffResult.rightBlockRanges.length;
  ...
  highlight() {
    const self = this;
    const allRanges = {};
    allRanges[`${DEL},${true}`] = this.diffResult.leftBlockRanges;
    allRanges[`${DEL},${false}`] = this.diffResult.leftInlineRanges;
    allRanges[`${INS},${true}`] = this.diffResult.rightBlockRanges;
    allRanges[`${INS},${false}`] = this.diffResult.rightInlineRanges;
    ...
    if (highlightLine) {
      return {
        ...
        options: {
          ...
          marginClassName: color.getLineColorClass(colorClass),
          // 高亮右侧的 minimap
          minimap: {
