...
export class Diff {
  ...
  // 排序。大小按优先级进行比较 (offset, diffType, length)
  static sort(diffs) {
    diffs.sort((a, b) => {
      const n = a.offset - b.offset;
      if (n != 0) {
        return n;
      } else if (a.diffType !== b.diffType) {
        return a.diffType === DEL ? -1 : 1;
      } else {
  ...
  // 输出 diff 区间的文本，用于 debug
  static fillText(diffs, leftEditor, rightEditor) {
    diffs.map((diff) => {
      const editor = diff.diffType === DEL ? leftEditor : rightEditor;
      diff.text = editor.text().slice(diff.offset, diff.offset + diff.length);
    });
    return diffs;
  }

  // 在控制台输出测试用例文本
  static genTestCase(diffs) {
    diffs.map((d) => {
      console.log(
        `new diff.Diff(${d.offset}, ${d.length}, ${d.diffType == DEL ? "diff.DEL" : "diff.INS"}, ${
          d.highlightLine
        }, [${d.path.map((s) => '"' + s + '"').join(", ")}]),`
      );
    });
...
export class Comparer {
  ...
  diffArray(ldata, rdata) {
    ...
    for (let i = 0; i < diffs.length; i++) {
      ...
      // 如果两边的数组元素不同，则递归比较
      if (idx === ridx && diffType === DEL && ndiff?.diffType === INS) {
  ...
  diffObject(ldata, rdata) {
    ...
    // 将左右两侧只有 key 存在差异的部分加入 diffs。WARNING: 存在重复比较，可优化成在全部比较完成后，单独遍历所有差异 key
