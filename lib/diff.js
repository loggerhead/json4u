...
export function semanticCompare(ltext, rtext) {
  ...
  return new SemanticDiffResult(diffs, isTextCompare, (lerrors.length && lerrors) || rerrors);
}

// 语义化差异
export class SemanticDiffResult {
  constructor(diffs, isTextCompare, errors) {
    this.diffs = diffs;
    // 是文本比较吗？
...
export class Comparer {
  ...
  diffObjectKey(diffs) {
    ...
    for (const delDiff of delHitDiffs) {
      ...
      const delPathStr = pointer.toPointer(delPath);

      for (const insDiff of insHitDiffs) {
        const rnode = insDiff.node;
        const insPath = rnode.path;
        const insPathStr = pointer.toPointer(insPath);
    ...
    {
      const dd = delHitDiffs.filter((d) => !seen.has(pointer.toPointer(d.node.path)));
      allDiffs.push(...dd);
    }
    {
      const dd = insHitDiffs.filter((d) => !seen.has(pointer.toPointer(d.node.path)));
      allDiffs.push(...dd);
    }
