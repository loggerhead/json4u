...
// TODO: 只考虑 object、array。其它情况走文本比较
export function diffEditors(leftEditor, rightEditor) {
  const ltree = new Tree(leftEditor);
  const rtree = new Tree(rightEditor);
  const comparer = new Comparer(ltree, rtree);
  comparer.compare();
  return comparer.diffs;
}
...
export class Tree {
  ...
  getValue(path) {
    return this.getNodeValue(this.getNode(path));
  }
...
export class Comparer {
  ...
  diffVal(ldata, rdata) {
    if (Array.isArray(ldata) && Array.isArray(rdata)) {
      ...
      if (isNeedDiffVal(ldata, rdata)) {
        ...
        const charDiffs = charDiff(ltext, rtext);
  ...
  diffArray(ldata, rdata) {
    // 数组维度的 diff
    const diffs = arrayDiff(ldata, rdata);
    ...
    leftOnlyKeys.forEach((lkey) => {
      rightOnlyKeys.forEach((rkey) => {
        if (!seen.has(rkey) && isNeedDiffKey(ldata, rdata, lkey, rkey)) {
          const charDiffs = charDiff(lkey, rkey);
          const diffs = this.newInlineDiffs(charDiffs);
          this.diffs.push(...diffs);
          seen.add(lkey);
          seen.add(rkey);
 ...
 */
export function charDiff(ltext, rtext, ignoreBlank = true) {
  let lpos = 0;
  let rpos = 0;
  const diffs = [];
 ...
 */
export function arrayDiff(ldata, rdata) {
  const lineArray = [""];
  const lineHash = {};
