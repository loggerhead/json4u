 ...
 * @property {boolean} nest 进行嵌套解析吗？
 * @property {boolean} format stringify 时进行格式化吗？
 * @property {boolean} forcePretty 强制使用 prettier 进行格式化吗？
 * @property {boolean} forceRawFormat 强制使用 prettier 进行格式化吗？
 * @property {number} printWidth prettier 格式化参考的字符数
 * @property {number} tabWidth prettier 格式化时，缩进的空白数
 * @property {"" | "asc" | "desc"} order 指定 stringify 排序的顺序
 */
...
export class Node {
  ...
  toCsvJSON() {
    ...
    for (const key of this.keyNodes.keys()) {
      const node = this.getValueNode(key);

      if (hasHeader === undefined) {
        hasHeader = node.isObject();
      }

      if (node.isObject() && hasHeader) {
        const obj = {};

        for (const key of node.keyNodes.keys()) {
          const s = node.getValueNode(key).stringify();
          obj[unquote(key)] = unquote(s);
        }

        result.push(obj);
      } else if (node.isArray() && !hasHeader) {
        const arr = [];
  ...
  stringify(options = {}) {
    ...
    const needFormat = Boolean(options?.format);

    const formatKVs = (kvs, vnodes, indentLevel) => {
      if (needFormat && kvs.length > 0) {
        const firstVnode = vnodes[0];

        if (kvs.length > 1 || firstVnode.isObject() || firstVnode.isArray()) {
          const lastIndent = repeat(tab, indentLevel);
          const indent = lastIndent + tab;
          return `\n${indent}${kvs.join(`,\n${indent}`)}\n${lastIndent}`;
        }
      }
    ...
    const doStringify = (node, indentLevel = 0) => {
      if (node.isObject()) {
        const ss = [];
        const vnodes = [];
        ...
        for (const key of keys) {
          ...
          const valueStr = doStringify(vnode, indentLevel + 1);
          vnodes.push(vnode);
          ss.push(needFormat ? `${key}: ${valueStr}` : `${key}:${valueStr}`);
        }

        return `{${formatKVs(ss, vnodes, indentLevel)}}`;
...
class JsonVisitor {
  ...
  complete() {
    this.root = this.currentParent || new Node();
  }
...
export function parseJSON(text, options = {}) {
  ...
  const node = visitor.root;
  node.errors = addContextToErrors(visitor.text, visitor.errors);
  return node;
}
...
export function isEqual(jsonStr1, jsonStr2) {
  const o1 = parseJSON(jsonStr1);
  const o2 = parseJSON(jsonStr2);
  return (o1.hasError() && o2.hasError()) || o1.isEquals(o2);
}
