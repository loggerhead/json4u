...
import {unescapeJSON} from "./escape";
...
class JsonVisitor {
  ...
  // TODO: 改正则。跳过空格
  isNeedNestParse(value) {
    return this.nest && typeof value === 'string' && (
  ...
  onLiteralValue(value, offset, length, startLine, startColumn, pathSupplier) {
    ...
    this.ensureGeneratedArrayKeyNode(offset);
    const node = this.isNeedNestParse(value) ? doParseJSON(value, this.nest) : this.newNode('value', offset, length);
    this.fillToken(node);
 ...
 * @param {boolean} nest - 嵌套解析 escape 的字符串
 * @returns {JsonVisitor} json visitor
 */
function doParseJSON(text, nest = false) {
  if (nest) {
    text = unescapeJSON(text);
  }

  const visitor = new JsonVisitor(text, nest);
...
export function parseJSON(text, nest = false) {
  const visitor = doParseJSON(text, nest);
