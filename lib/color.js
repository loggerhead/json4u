...
export function getColorClass(diffType, inline = false) {
  return filterByType(inline ? inlineClasses : hunkClasses, diffType).lineClass;
}

export function getLineClass(colorClass) {
  return (colorClass.includes("del") ? filterByType(hunkClasses, DEL) : filterByType(hunkClasses, INS)).lineClass;
}

export function getMarginClass(colorClass) {
  return (colorClass.includes("del") ? filterByType(hunkClasses, DEL) : filterByType(hunkClasses, INS)).marginClass;
}

export function getMinimapColor(colorClass) {
  let v = filterByType(hunkClasses, colorClass)?.value;
  if (v) {
    return v;
  }

  return filterByType(inlineClasses, colorClass)?.value;
}

export function getOverviewRulerColor(colorClass) {
  return (colorClass.includes("del") ? filterByType(inlineClasses, DEL) : filterByType(inlineClasses, INS)).value;
}
