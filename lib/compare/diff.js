...
export class DiffResult {
  ...
  static newFromObj(obj) {
    const r = new DiffResult([]);
    Object.assign(r, obj);
    return r;
  }

  countLines() {
    const ln = this.ldiffs.reduce((acc, d) => {
      return acc + (d.range ? (d.range.endLineNumber - d.range.startLineNumber + 1) : 0);
    }, 0);

    const rn = this.rdiffs.reduce((acc, d) => {
      return acc + (d.range ? (d.range.endLineNumber - d.range.startLineNumber + 1) : 0);
    }, 0);
...
export class HunkDiff extends Diff {
  constructor(offset, length, type, inlineDiffs = []) {
    super(offset, length, type);
    this.inlineDiffs = inlineDiffs;
  }
}
