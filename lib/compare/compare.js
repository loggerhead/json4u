...
import {classify, DEL, DiffResult, HunkDiff, InlineDiff, INS} from "./diff";
...
export function smartCompare(ltext, rtext, needTextCompare = false) {
  ...
  if (!needTextCompare) {
    [ltree, lerrors] = parseJSON(ltext);
    [rtree, rerrors] = parseJSON(rtext);
  }

  // JSON 解析失败时，走文本比较
  const isTextCompare = needTextCompare || Boolean(lerrors.length || rerrors.length);
  const diffs = isTextCompare ? compareHunkTexts(ltext, rtext) : new Comparer().compare(ltree, rtree);
...
export class Comparer {
  ...
  diff(lnode, rnode) {
    ...
    // 类型一样时，进行行内比较
    if (lnode.type === rnode.type && ltext !== rtext) {
      const [ldiffs, rdiffs] = classify(compareInlineTexts(ltext, rtext));
      ldiff.inlineDiffs = this.newInlineDiffs(lnode, ldiffs);
      rdiff.inlineDiffs = this.newInlineDiffs(rnode, rdiffs);

      if (ldiff.inlineDiffs.length === 0) {
        ldiff = undefined;
      }
      if (rdiff.inlineDiffs.length === 0) {
        rdiff = undefined;
      }
    }
...
export function compareHunkTexts(ltext, rtext) {
  // 行内比较
  const inlineCompare = (ldiff, rdiff) => {
    const lpos = ldiff.offset;
    const rpos = rdiff.offset;
    const lstr = ltext.slice(lpos, lpos + ldiff.length);
    const rstr = rtext.slice(rpos, rpos + rdiff.length);
    const diffs = compareInlineTexts(lstr, rstr).map((d) => {
      d.offset += d.type === DEL ? lpos : rpos;
      return d;
    });

    return new DiffResult(diffs).classify();
  };
  ...
  const diffs = jsdiff
    ...
    .map((d) => {
      ...
      // 左侧删除
      if (d.removed) {
        diff = new HunkDiff(lpos, n - 1, DEL);
        last = diff;
        lpos += n;
        // 右侧新增
      } else if (d.added) {
        diff = new HunkDiff(rpos, n - 1, INS);

        // 如果左侧是删除，右侧是新增，说明是替换。此时进行文本比较，生成行内差异
        if (last && last.type === DEL) {
          [last.inlineDiffs, diff.inlineDiffs] = inlineCompare(last, diff);
        }
...
export function compareInlineTexts(ltext, rtext) {
  ...
  const diffs = jsdiff
    .diffChars(ltext, rtext)
    .map((d) => {
      let diff;
      const n = d.value.length;

      if (d.removed) {
        diff = new InlineDiff(lpos, n, DEL);
        lpos += n;
      } else if (d.added) {
        diff = new InlineDiff(rpos, n, INS);
        rpos += n;
      } else {
        lpos += n;
        rpos += n;
...
export function compareArray(lvals, rvals) {
  ...
  jsdiff.diffArrays(lchars, rchars).forEach((d) => {
    if (d.removed || d.added) {
      for (let i = 0; i < d.count; i++) {
        diffs.push(d.removed ? new HunkDiff(l++, 1, DEL) : new HunkDiff(r++, 1, INS));
      }
    } else {
      l += d.count;
      r += d.count;
