...
export function compareArray(lvals, rvals) {
  ...
  compareInlineTexts(lchars, rchars).forEach((d) => {
    if (d.type === DEL || d.type === INS) {
      for (let i = d.offset; i < d.offset + d.length; i++) {
        diffs.push(new HunkDiff(i, 1, d.type));
      }
    }
  });
