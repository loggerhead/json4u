...
import {csv2json, json2csv} from "@/lib/csv";
...
self.onmessage = async (event) => {
  ...
  switch (cmd) {
    case "format": {
      ...
      callback(await format(text, options));
      return;
    }
    case "minify":
    ...
    case "csv2json": {
      const {text, options} = data;
      callback(await csvToJsonString(text, options));
      return;
    }
    case "json2csv": {
      const {text, options} = data;
      callback(await json2csv(text, options));
      return;
    }
    case "compare": {
      const {ltext, rtext, options} = data;
      callback(compare(ltext, rtext, options));
      return;
    }
    case "handlePaste": {
      const {ltext, rtext, options} = data;
 ...
 * @return {{text, errors, diffResult}} 返回值
 */
async function handlePaste(isLeft, ltext, rtext, options) {
  ...
  // 右侧编辑器粘贴文本且左右两侧编辑器都不为空时，进行比较
  if (!isLeft && ltext.length > 0 && rtext.length > 0) {
...
async function csvToJsonString(s, options) {
  const {object, error} = csv2json(s, options);
  const jsonStr = JSON.stringify(object);
  const text = await format(jsonStr, options);
